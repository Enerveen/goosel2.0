...
export class BoidEntity extends Entity {
    ...
    private dodge(entity: Entity, precalculatedDistance?: number) {
        const distance = precalculatedDistance || new Vector2(this.position.x - entity.position.x, this.position.y - entity.position.y).norm();
    ...
    update(elapsedTime: number) {
        if (this.centerPoints.length) {
            ...
            const unit = new Vector2(centerPoint.x - this.position.x, centerPoint.y - this.position.y).normalized();
