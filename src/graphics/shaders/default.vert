...
void main() {
    ...
    bendAge = bendData;
    v_bendAmplitude = isGrass ? mix(0.0, 0.65, bendAge) : 0.f;
    v_windBendAmplitude = 0.2 + 0.53 * (0.5 * cos(0.02 * float(u_time) + 5.0 * v_worldPosition.y / u_resolution.y) + 0.5);
    v_bend = v_bendAmplitude * bendAge * sin(4.0 * 2.0 * 3.1415 * bendAge + 0.07f * float(u_time)) + v_windBendAmplitude * sin(0.002 * float(u_time) + 13.0 * v_worldPosition.x / u_resolution.x);
